 ...
 * File:       ToStringUtil
 * Desc:       java 原生的 toString 工具类: 反序列化
 *
 * @author wangchuangfeng
 * CreateTime: 2023-01-11 16:36
 */

public class JavaToStringUtils {
    /**
     * toString格式反序列化
     */
    ...
    public static <T> List<T> toObjectList(Class<T> clazz, String str) throws Exception {
        if (str.startsWith("[")) {
            str = str.substring(1);
        }
        if (str.endsWith("]")) {
        ...
        List<String> pieces = Splitter.on(",").trimResults().splitToList(str);
        List<T> list = new ArrayList<>();
        for (String piece : pieces) {
            T t = toObject(clazz, piece);
            list.add(t);
        }
        return list;
    }
